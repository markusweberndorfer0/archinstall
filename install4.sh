...
# Config bumblebee
sed -i 's/status_command i3status/status_command bumblebee-status -m time cpu memory battery nic pulseout xrandr -p pulseout.right-click="pavucontrol" xrandr.right-click="arandr" -t solarized-powerline -p interval=5s\n        position top/' ~/.config/i3/config
